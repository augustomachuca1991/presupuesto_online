...
export function BuscadorGenerico({
  ...
  if (selected) {
    return (
      <div className="mb-4">
        {labelSeleccionado && (
          <label className="block text-[11px] font-semibold text-antm uppercase tracking-widest mb-1.5">{labelSeleccionado}</label>
        )}
        <div className="flex items-center gap-2.5 px-3 py-2.5 bg-ant2 rounded border border-border">
  ...
  return (
    <div className="mb-4">
      {label && <label className="block text-[11px] font-semibold text-antm uppercase tracking-widest mb-1.5">{label}</label>}
      ...
      <div className="flex gap-2">
        <div className="relative flex-1" ref={wrapRef}>
          <button
            ...
            className={`w-full flex items-center gap-2 px-3 h-9 rounded-md border text-left text-[13px] transition-all cursor-pointer
              ${open ? "border-yel ring-1 ring-yel/30 bg-ant2" : "border-white/20 bg-ant2 hover:border-antm"}`}
          >
            <i className={`${icono} text-[15px] text-antm shrink-0`} />
            <span className="flex-1 text-antm select-none">{placeholderTrigger}</span>
            <i className={`${ICONS.CHEVRON_DOWN} text-[13px] text-antm transition-transform duration-150 ${open ? "rotate-180" : ""}`} />
          </button>

          {open && (
            <div className="absolute z-40 left-0 right-0 top-[calc(100%+4px)] bg-ant2 border border-white/20 rounded shadow-lg overflow-hidden" role="listbox" aria-label={label}>
              <div className="p-2 border-b border-border">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-8 pr-3 h-8 rounded-md border border-white/20 text-[12px] text-antl bg-ant2 outline-none focus:border-yel focus:ring-1 focus:ring-yel/30 transition ${inputClassName}`}
                    autoComplete="off"
                    role="searchbox"
                  ...
                  items.map((item, idx) => (
                    <li key={itemKey(item)} id={`buscador-opt-${idx}`} data-index={idx} role="option" aria-selected={highlightedIndex === idx}>
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 flex items-center gap-2.5 transition-colors cursor-pointer ${highlightedIndex === idx ? "bg-ant" : ""}`}
                      >
                        {renderItem(item)}
                      </button>
                    </li>
              ...
              <div className="px-3 py-1.5 border-t border-border bg-ant">
        ...
        <button
          type="button"
          onClick={onNuevo}
          className="bg-yel text-yeld text-[13px] font-medium px-3.5 h-9 rounded-md flex items-center gap-1.5 hover:bg-yelm cursor-pointer shrink-0 transition-colors"
          title={nuevoTitle}
        >
