...
function EmptyState({ hayFiltros }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
      <div className="w-16 h-16 rounded-2xl bg-[#1c1c1e] border border-[#2e2e30] flex items-center justify-center">
        <i className={`ti ${hayFiltros ? "ti-filter-off" : "ti-car-off"} text-[28px] text-[#555]`} />
      </div>
      <div>
...
export default function VehiculosPage() {
  ...
  const { vehiculos, isLoading, refetch: cargarVehiculos, marcas, modelos, cargarModelos, crearVehiculo, editarVehiculo, eliminarVehiculo } = useVehiculos();
    ...
    <>
      ...
      <div className="max-w-[620px] mx-auto px-3 sm:px-4 pt-4 pb-12">
        ...
        <div className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl mb-5 shadow-sm border border-border">
          <div className="flex items-center justify-center w-9 h-9 rounded-xl bg-[#1c1c1e] shrink-0">
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1.5 bg-yel text-yeld text-[13px] font-semibold
                       px-3.5 h-9 rounded-lg hover:bg-yelm transition-colors cursor-pointer shrink-0"
          >
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-8 h-9 rounded-md border border-border bg-white text-[13px] text-ant
                           placeholder:text-ant3 focus:outline-none focus:border-yel focus:ring-1 focus:ring-yel/20
            ...
            <button
              ...
              className={`h-9 px-3 rounded-md border text-[13px] flex items-center gap-1.5 transition-colors
        ...
        {isLoading ? (
          <div className="flex flex-col gap-2">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-[66px] bg-[#1c1c1e] rounded-xl border border-[#2e2e30] animate-pulse" />
            ))}
          </div>
